...
if (!$connection) {
    ...
    $id=$_SESSION['id'];
$sql= "update user_info set is_active_doc2='on', is_active_doc1='off' where id='$id'";
...
function checkEdits() {
  ...
  if(localStorage.userEdits!=null)
    document.getElementById("edit").innerHTML = localStorage.userEdits2;
}
...
<?php
    ...
    $sql= "select * from user_info where is_active_doc2='on'";
    $result=mysqli_query($connection,$sql);
...
<?php  $connection=new mysqli("localhost","root","","user");   //create connection 
    ...
    $sql= "select * from user_info where is_active_doc2='off'";
    $result=mysqli_query($connection,$sql);
    //$r=mysqli_fetch_array($result);
    $count = mysqli_num_rows($result);
    while ($row = mysqli_fetch_array($result)) {
